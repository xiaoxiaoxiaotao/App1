...
import (
	"Docker-common_error_option1/functions"
	"context"
	"encoding/json"
	"fmt"
	"log"
	...
	r.POST("/api", func(c *gin.Context) {
		...
		if err := c.ShouldBindJSON(&req); err != nil {
			...
			c.JSON(http.StatusBadRequest, gin.H{
				"result":     "Error parsing input JSON. Ensure correct format.",
				"time_taken": 0,
		...
		// use function to handel event
		failedTask := functions.LambdaHandler(event)
